...
AI コーディングアシスタント（Claude Code、OpenAI Codex）をプリインストールした汎用開発用 DevContainer 環境を提供する。開発者はコンテナを起動するだけで、ローカル環境を汚さずに AI 支援開発を開始できる。
...
- AI コーディングアシスタント（Claude Code, OpenAI Codex）のプリインストール
...
全ツールはコンテナイメージのビルド時にネイティブバイナリとしてインストールされ、npm / Node.js は不要。コンテナ起動時の追加インストールも不要。
...
| FNC-003 | AI コーディングアシスタント実行 | Claude Code（メイン）、OpenAI Codex の実行 |
