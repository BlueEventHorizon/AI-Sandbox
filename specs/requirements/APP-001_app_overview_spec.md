...
AI コーディングアシスタント（Claude Code、Gemini CLI、OpenAI Codex）をプリインストールした汎用開発用 DevContainer 環境を提供する。開発者はコンテナを起動するだけで、ローカル環境を汚さずに AI 支援開発を開始できる。
...
| 目的 | 成功基準 |
| --- | --- |
| AI ツールが即時利用可能 | コンテナ起動後、追加のインストール作業なしで `claude --help` が正常終了すること |
| 環境の再現性 | 同一の Dockerfile から異なるマシンでビルドしたイメージが同一のツールセットを含むこと |
| OS 非依存 | macOS, Windows (WSL), Linux のいずれでもコンテナが起動し、AI ツールが動作すること |
...
- AI コーディングアシスタント（Claude Code, Gemini CLI, OpenAI Codex）のプリインストール
...
- CI/CD パイプラインへの統合
- Windows / Linux 向けの Docker 環境自動セットアップ（Docker Desktop の利用を前提とする）
...
| --- | --- | --- |
| macOS（Intel / Apple Silicon） | Colima（推奨）または Docker Desktop | `make install` で自動セットアップ（FNC-004） |
| Windows（WSL 2 必須） | Docker Desktop + WSL 2 | Docker Desktop の手動インストール |
...
| Docker Desktop（Windows / macOS） | 4.0 以上 | Docker Engine 20.10 相当を含む |
| VS Code | DevContainers 拡張機能の要件に準じる | DevContainers 拡張機能が対応する VS Code バージョンに従う |

## 前提条件

- Docker（または Colima 等の互換ランタイム）がインストールされ、起動していること（Docker Engine 20.10 以上）
- VS Code を利用する場合は DevContainers 拡張機能がインストールされていること（VS Code のバージョンは DevContainers 拡張機能の要件に準じる）
...
- **ベースイメージ**: Alpine Linux ベースの Node.js 環境（軽量。Gemini CLI の実行に Node.js が必要なため）
...
| ツール | 役割 | インストール方式 |
| --- | --- | --- |
| Claude Code | メインの AI コーディングアシスタント | ネイティブバイナリ |
| Gemini CLI | AI コーディングアシスタント（Claude Code から MCP 経由でも利用可能） | グローバル npm パッケージ |
| OpenAI Codex | AI コーディングアシスタント | ネイティブバイナリ |

全ツールはコンテナイメージのビルド時にインストールされ、コンテナ起動時の追加インストールは不要。
...
### シナリオ 2: Windows / Linux で初めから始める（Docker 環境なし）

| 手順 | 操作 | 備考 |
| --- | --- | --- |
| 1 | Docker Desktop（Windows）または Docker Engine（Linux）をインストール | 手動セットアップ |
...
| シナリオ 2: Windows / Linux 初回セットアップ | Docker Desktop / Docker Engine の手動インストール | （スコープ外。手動インストールを前提） |
...
| FNC-003 | AI コーディングアシスタント実行 | Claude Code（メイン）、Gemini CLI（MCP 経由）、OpenAI Codex の実行 |
...
| npm レジストリ（@google/gemini-cli） | Gemini CLI のインストール | Gemini CLI が利用不可になる。Claude Code と Codex は影響なし |
...
| Docker Hub（node:24-alpine） | ベースイメージ | 別のレジストリまたはローカルキャッシュから取得する必要がある |
...
| Node.js ランタイムをコンテナに含める | Gemini CLI が Node.js に依存するため。Claude Code と Codex はネイティブバイナリで Node.js 不要 |
| macOS 自動セットアップは Homebrew を前提とする | Colima / Docker CLI のインストールに Homebrew を使用するため |
| Windows / Linux の Docker 環境自動セットアップは未提供 | Docker Desktop または Docker Engine の手動インストールを前提とする |
...
| Alpine Linux での AI ツール互換性問題 | 中 | musl 環境固有の問題が発生する可能性がある。報告があればベースイメージの変更（Debian 系）を検討する |
...
| 用語 | 定義 |
| --- | --- |
| AI コーディングアシスタント | 本プロジェクトが対象とする AI 開発支援ツール（Claude Code, Gemini CLI, OpenAI Codex）の正式な総称 |
| DevContainer | VS Code の Dev Containers 拡張機能が管理するコンテナ開発環境 |
| MCP | Model Context Protocol。AI コーディングアシスタント間の連携プロトコル |
